...
-- ? NUEVA IMPLEMENTACIÓN: PARTIDO PARTICIONADO POR JORNADA ?

-- 1?? Crear la tabla `PARTIDO` como particionada por `jornada`
CREATE TABLE PARTIDO (
    idPartido       NUMBER(20), -- ? Se quita la PRIMARY KEY (se agrega después como índice único)
...
PARTITION BY RANGE (jornada) (
    PARTITION PARTIDO_HISTORICO VALUES LESS THAN (3375),
    PARTITION PARTIDO_ULTIMA_TEMPORADA VALUES LESS THAN (3399),
    PARTITION PARTIDO_FUTURO VALUES LESS THAN (MAXVALUE)
);

-- ?? Crear un índice único global en `idPartido` porque no se puede usar PRIMARY KEY en tablas particionadas
CREATE UNIQUE INDEX idx_partido_id ON PARTIDO (idPartido) GLOBAL;


-- 3?? Secuencia y Trigger para ID de `PARTIDO` (Se mantiene sin cambios)
